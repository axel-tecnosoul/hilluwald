...
if (file_exists ($file)){//validacion para saber si el archivo ya existe previamente
  ...
  while($param==0){//mientras el parametro siga en 0, no ha encontrado un NULL, entonces sige metiendo datos
    ...
    if($objPHPExcel->getActiveSheet()->getCell('A'.$i)->getCalculatedValue()==NULL or $i==$f){//pregunta si ha encontrado un valor null en la columna
      ...
      //clientes
      {
        $id=str_replace("'"," ",$objPHPExcel->getActiveSheet()->getCell('A'.$i)->getCalculatedValue());
        $email=str_replace("'"," ",$objPHPExcel->getActiveSheet()->getCell('B'.$i)->getCalculatedValue());
        $clave=$dni=str_replace("'"," ",$objPHPExcel->getActiveSheet()->getCell('C'.$i)->getCalculatedValue());
        $nombre=str_replace("'"," ",$objPHPExcel->getActiveSheet()->getCell('D'.$i)->getCalculatedValue());
        $apellido=str_replace("'"," ",$objPHPExcel->getActiveSheet()->getCell('E'.$i)->getCalculatedValue());
        $activo=1;
        $fecha_alta=str_replace("'"," ",$objPHPExcel->getActiveSheet()->getCell('F'.$i)->getCalculatedValue());
        var_dump($fecha_alta);
        if($fecha_alta==""){
          $fecha_alta=NULL;
        }else{
        ...
        var_dump($fecha_alta);
        $telefono=str_replace("'"," ",$objPHPExcel->getActiveSheet()->getCell('G'.$i)->getCalculatedValue());
        $credito=0;

        $sql2 = "INSERT INTO clientes (id, email, clave, dni, nombre, apellido, activo, fecha_alta, telefono, credito) VALUES (?,?,?,?,?,?,?,?,?,?)";
        $sql2 = $pdo->prepare($sql2);
        $sql2->execute([$id,$email,$clave,$dni,$nombre,$apellido,$activo,$fecha_alta,$telefono,$credito]);
