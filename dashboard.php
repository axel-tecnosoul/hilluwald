				  ...
				  <div class="card-body">
					<div class="chart-widget-dashboard">
					  <div class="media">
						...
						$pdo = Database::connect();
						$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
						$sql = "SELECT count(*) cant FROM `clientes` WHERE activo = 1 ";
						$q = $pdo->prepare($sql);
						$q->execute();
						$data = $q->fetch(PDO::FETCH_ASSOC);
						$clientesActivos = $data['cant'];
						Database::disconnect();
						?>
						<div class="media-body">
						  <h5 class="mt-0 mb-0 f-w-600"><span class="counter"><?php echo $clientesActivos; ?></span></h5>
						  <p><a href="listarClientes.php">Clientes activos</a></p>
						</div>
					  </div>
					</div>
				  </div>
