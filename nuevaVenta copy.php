    ...
    foreach ($aFacturas as $factura) {
      ...
      foreach ($factura as $iva => $monto) {
        ...
        $aIva[]=array(
          ...
          'BaseImp'=> 0,//100, // Base imponible -> ES IGUAL A ImpNeto?
          'Importe'=> number_format($ImpIVA,2,".",""),//21 // Importe -> ES IGUAL A ImpIVA?
        );
      }

      $monto_ingresos_brutos=0;
      ...
      if($tipo_comprobante_bbdd=="A"){
        ...
        $monto_percepcion_iva=$ImpNeto-($ImpNeto/(($porcentaje_percepcion_iva/100)+1));//1.21
      }

      $monto_tributos=$monto_ingresos_brutos+$monto_percepcion_iva;

      $ImpNeto-=$monto_tributos;

      foreach ($aIva as $key => $value) {
        $aIva[$key]["BaseImp"]=number_format($ImpNeto,2,".","");
      }
      ...
      if($tipo_comprobante_bbdd=="A"){
        var_dump($monto_ingresos_brutos);
