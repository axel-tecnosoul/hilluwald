...
class PDF extends FPDF{
  ...
  /**
   ...
   function copia($emision){
    ...
    /** Venta */
    $sql = "SELECT f.id, date_format(f.fecha_cbte,'%d/%m/%Y') AS fecha, f.punto_venta, f.cae, f.fecha_vto_cae, f.numero_cbte, f.tipo_comprobante, f.total_bruto, f.total_neto, f.id_iva, f.alicuota_ingresos_brutos, f.total_ingresos_brutos, c.razon_social, c.direccion, c.cuit, c.cond_fiscal, v.id AS id_venta, tipo_doc, dni FROM facturas f INNER JOIN ventas v ON f.id_venta=v.id INNER JOIN clientes c ON v.id_cliente=c.id WHERE f.id = ?";
    $q = $pdo->prepare($sql);
    $q->execute(array($id));
    //$q->debugDumpParams();
    ...
    if(in_array($tipo_comprobante,["A","NCA"])){
      ...
      $this->Cell(40, 8, utf8_decode("$".number_format($data["total_ingresos_brutos"],2, ',', '.')), 0, 0, 'R', 1);
      $this->Ln();
