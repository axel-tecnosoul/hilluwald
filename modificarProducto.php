...
if ( !empty($_POST)) {
  ...
  $sql = "UPDATE productos set descripcion = ?, precio = ?, iva = ?, activo = ?, id_usuario_alta_modificacion = ? where id = ?";
  $q = $pdo->prepare($sql);
  $q->execute(array($_POST['descripcion'],$_POST['precio'],$_POST['iva'],$_POST['activo'],$_SESSION["user"]["id"],$_GET['id']));
  ...
  $pdo = Database::connect();
  $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
  $sql = "SELECT id, descripcion, precio, iva, activo FROM productos WHERE id = ? ";
  $q = $pdo->prepare($sql);
  $q->execute(array($id));
                          ...
                          <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                              <select name="iva" id="iva" class="form-control" required>
                                ...
                                <option value="10.5" <?php if ($data['iva']==10.5) echo " selected ";?>>10.5%</option>
                                <option value="21" <?php if ($data['iva']==21) echo " selected ";?>>21%</option>
                              </select>
                            </div>
                          </div>
                          <div class="form-group row">
